...
LOOP = True  # если True — верх и низ соединены по вертикали
...
def step_single_lane(positions, speeds):
    ...
    rand = np.random.random(size=v_new.shape)
    ...
    for lane in range(N_LANES):
        ...
        for i in range(len(new_positions[lane]) - 1, -1, -1):
            ...
            if 0 <= left < N_LANES and (left != lane):
                ...
                if stim_left and safe_left and np.random.random() < P_LC:
            ...
            if 0 <= right < N_LANES and (right != lane):
                ...
                if stim_right and safe_right and np.random.random() < P_LC:
...
def run_sim_multi(density):
    ...
    for _ in range(n_cars_total):
        lane = int(np.random.randint(0, N_LANES))
        pos = int(np.random.randint(0, L))
...
def create_animation(history_segment, filename='traffic_multi_lane.gif'):
    ...
    # Уменьшаем поля, но не убираем полностью
    plt.subplots_adjust(
        left=0.03,   # поле слева
        right=0.97,  # справа
        top=1,    # сверху — оставляем место для заголовка
        bottom=0.0  # снизу
    )

    def update(frame):
        im.set_data(data[frame])
        ax.set_title(f'Шаг {frame}', color='black', pad=20)  # немного отступа от картинки
        return [im]
